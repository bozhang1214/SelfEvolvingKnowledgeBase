...
## 2026-09-18（M2 第六批：UI 人工路径验收）

- **改动**：
  - 云端地址支持**构建期覆盖**（`-PsekbBaseUrl=…`），UI 联调不必在模拟器上手打地址。
  - 修 UI 验收抓到的两个真缺陷：**密码框明文显示** → `PasswordVisualTransformation`；
    **徽标漏报客户端侧升级**（服务端 `execution.escalated` 只统计服务端内部升级，
    端侧失败改道时显示成"云端完成"）→ 徽标同时看客户端结果 + 5 条 `BadgeTest`。
- **验证**：单测 **96 用例**全绿；界面真实点击走通「接入设备 → 发送 → 回答 + 执行位置徽标」，
  截图存 `docs/screenshots/m2-ui-e2e.png`；环境坑（输入法遮挡、`input text` 空格、TAB 切焦点）
  写进 §1.5。

## 2026-09-18（M2 第四批：工具调用合法率对照实验）

- **改动**：新增 `eval/ToolCallEvalRunner.kt` 与 `--ez eval [--ez hard true]` 入口：
  同一批提示词 × 两档模型 × 约束解码 ON/OFF，统计合法率与延迟。
- **结果（40 次调用）**：**四种条件全部 100% 合法**，无工具名幻觉；延迟差在噪声范围。
  结论是"约束解码在这批条件下没有可测量收益"——**推翻了"小模型必须靠 grammar"的初始假设**，
  因此建议先不为 grammar 付工程复杂度（细节与实验局限见 `docs/VERIFICATION.md` §1.4）。

## 2026-09-18（M2 第三批：模拟器真机 E2E 打通）

- **改动**：
  - `ui/`：Compose 界面（聊天 + 执行位置徽标 + 最近决策 + 权限审计面板 + 端云地址与设备接入）。
  - `SelfTest.kt`：可脚本化的端侧自检（`am start --ez selftest true` + logcat），
    14 项覆盖端侧链路与云端协议；把"点屏幕看结果"变成可留存、可复核的证据。
  - 修 `timeout` 升级信号是**死代码**（编排器从没把首字耗时喂给守卫）。
  - 新增端侧预热：同一任务 **TTFT 1780ms → 83ms**（21 倍）。
  - 修登录字段名（`token` 而非 `access_token`）——服务端 200 却报"登录失败"。
- **验证**：单测 91 用例全绿；模拟器 E2E **14/14 PASS**（含真实 Ollama 流式、
  权限拦截 50%、云端 SSE 执行位置、路由事件落库）。详见 `docs/VERIFICATION.md` §1.3。
