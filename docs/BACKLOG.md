...
| D10 | embedding 模型缓存持久化 | 当前 `~/.cache/huggingface` 不在卷里，每次重建后端容器都要从 hf-mirror 重下 ~100MB 模型（启动慢、依赖外网）。建议挂专用卷 `sekb_hf_cache` 或把模型打进镜像；否则一旦 hf-mirror 不可用，重嵌入恢复会失败 | 中 |
