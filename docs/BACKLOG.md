...
| **D17** | **`DEVICE_ONLY` 语义收口：LLM 平面缺"本机"判据** | 嵌入侧已有 `EmbeddingProvider.isOnDevice` 闸门（`Retriever` 会拒），但 **LLM 平面的 EDGE 只按 `edgeBaseUrl` 寻址**——地址可能是**另一台机器**（Android 模拟器今天就指向 `10.0.2.2` = 开发机），于是"永不出设备"的数据实际出了设备。**建议在 M4 一起做（0.5–1 天）**：给 EDGE 加"本机"判据（回环/本机地址 ⇒ on-device；远端 ⇒ 拒绝 DEVICE_ONLY 请求并留痕 `escalation_blocked:not_on_device`）+ 一条契约夹具。这是**隐私硬边界名不副实**的问题，与桌面做不做无关 | 中（建议 M4 内做） |
