...
| D14 | 多功能联动（skill 调度 + 反馈闭环） | ✅ 用户画像地基已就绪（`5d9eefc`）。待做：聊天框下 skill 按钮（应聘助手/科技资讯助手/通用助手，参考豆包）→ 后端按 skill 路由到对应 agent + 注入画像/模块知识 → 聊天偏好回流更新画像 → 画像反向影响职位采集（关键词/城市/薪资）与资讯关注大类 | 中 |
