...
> 把跨台账重复的需求合并为「可一起处理」的集群，避免多套台账漂移。详见本地讨论区 `docs/tmp/` 的「简历补强与 SEKB 增强待办」（**未进版本库**，仅本机可见）。<!-- check-docs:ignore -->
...
| D14 | 多功能联动（skill 调度 + 反馈闭环） | ✅ 已闭环，但**形态已变更**：用户画像地基（`5d9eefc`）+ 画像反向影响职位分析（批量/单职位注入实时画像）+ 反馈闭环。⚠️ 其中的 **skill 按钮与调度已于 2026-09 整体移除**（前端 `c9d948a` / 后端 `daa76bd`），画像回流改由 `<PREF>` 标签内联触发（方案 A）；方案 B 记录员 LLM 后台抽取（`fd395f2`）**当前无调用方**，待「求职意图识别」接入（见 11-EVOLUTION）。**资讯部分定案：保持公共流，日报不做个性化**（`b8e964d`） | 已定案（skill 部分已作废） |
| D15 | 文档守卫工具（lychee 死链检查、OpenAPI/清单校验、文档结构守卫脚本） | 属于「文档工程剩余」（C1/C3/C5/C9-C10），统一推迟到文档专项轮次；本次测试工具链批次只接代码质量类工具，避免范围膨胀 | 低 |
| **D16** | **桌面端（Windows / Ubuntu）→ 本轮搁置** | owner 2026-09-21 决定搁置；**结论与代价已存证**，不要重新调研。审计要点：① 价值是**算力宿主**（给手机供算力）+ 文件主场，不是"第五个客户端"；② 宿主人日 **4.5–6.5**（原估 2–3 偏低：鉴权默认值、资源上限、Windows 服务化、卸载）；③ 关键修正 **拓扑 A≠B**——M0 profile 的 `planes.edge.base_url=127.0.0.1` 对手机毫无用处；④ Ollama 的 OpenAI 兼容端点**无 API key**，首版只能靠"默认绑 127.0.0.1 + Tailscale"隔离；⑤ CMP 桌面**不支持交叉编译**（`.msi` 只能在 Windows 打、`.deb` 只能在 Linux 打），本机是 macOS → GUI 后置；⑥ 原生 UI 路线下 GUI **建议直接不做**（浏览器入口为终点），可省 5–8 人日。详见 [`多端跨端-桌面端方案审计.md`](多端跨端-桌面端方案审计.md)。**重启前置条件**：先做 D17 + 有 Windows/Ubuntu 验证手段（VM 或 CI runner） | 搁置（有前置条件） |
| **D17** | ✅ **已解决（2026-09-21）**：`DEVICE_ONLY` 语义收口（LLM 平面的"本机"判据） | 嵌入侧已有 `EmbeddingProvider.isOnDevice` 闸门（`Retriever` 会拒），但 **LLM 平面的 EDGE 只按 `edgeBaseUrl` 寻址**——地址可能是**另一台机器**（Android 模拟器今天就指向 `10.0.2.2` = 开发机），于是"永不出设备"的数据实际出了设备。**建议在 M4 一起做（0.5–1 天）**：给 EDGE 加"本机"判据（回环/本机地址 ⇒ on-device；远端 ⇒ 拒绝 DEVICE_ONLY 请求并留痕 `escalation_blocked:not_on_device`）+ 一条契约夹具。这是**隐私硬边界名不副实**的问题，与桌面做不做无关 | ✅ 已解决（M4 第 3 步，191 测试） |
| **D18** | **端侧「设备适配」类策略（机型/SoC/内存档位映射、按 ROM 的差异开关）→ 只预留，本期不实现** | owner 2026-09-21：**L1 策略热修本期正常实现**（下发/缓存/签名/灰度/回滚/审计/白名单 + `embeddingSpace` 绑定，排期 M4.5，见 [`多端跨端-工程议题（网络层·包体·热修复）.md`](多端跨端-工程议题（网络层·包体·热修复）.md) §3.2）；**只有"设备适配"这一子集预留**：策略包 schema 里保留 `deviceProfiles: {}` 空对象占位，端侧白名单校验明确跳过它，代码不读取。**为什么本期不做**：① 没有真机矩阵，现在写规则全是猜；② 真机到货后要按实测数据定档，规则会全改。**触发条件**：真机到手并采集到机型/内存/降频差异数据之后 | 预留（有触发条件） |
...
> 总纲：本地讨论区 `docs/tmp/` 的「SEKB 深度代码重构方案」（**未进版本库**，仅本机可见；该轮 M1–M4 已完成，落地记录见 `docs/CHANGELOG.md`）。每 WP 独立提交 + 增量测试 + 同步 docs/tech；里程碑末部署。跟踪项逐条复核见 `docs/codeReview/未修复问题跟踪.md`「三、2026-09-09 复核表」。
...
## 端侧 RAG 后续（2026-09-20 记）

| 项 | 为什么 | 建议做法 |
|---|---|---|
| **UI 导入的最后一公里未脚本化验证** | 自检覆盖了"导入→检索→删除"逻辑，但"在选择器里选中文件→列表刷新"这一段没跑通（adb 推送的文件不在 SAF 的 Recent 列表；该 AVD 的 DocumentsUI 根目录抽屉对点击/滑动无响应） | 人工在模拟器上点两下即可确认；若要长期自动化，走 **UiAutomator 的 DocumentsUI 选择器**或改用 `ACTION_OPEN_DOCUMENT` + 预置 MediaStore 记录 |
| ~~PDF 文本层解析~~ | ✅ **2026-09-20 完成**（PdfBox-Android，见 VERIFICATION §1.8） | — |
| 扫描件 OCR | PDF 已支持**文本层**；扫描件（无文本层）目前如实拒绝 | 端侧 OCR（如 Tesseract/ML Kit）会让模型再大一圈，值得单独评估 |
| Word / Excel 解析 | 尚未支持（后端有 python-docx/pypdf，端侧没有） | 待真机出现真实用例再排 | 
| 文档管理界面（搜索/批量删除/重命名） | 现在只能列最近 3 份 | 等真机出现真实用量再做，避免凭空设计 |
| 文档 id 策略（当前=文件名） | 同名不同内容的文件会互相覆盖 | 真机出现用例后改成"文件名+内容哈希"，并给出重复导入提示 |
