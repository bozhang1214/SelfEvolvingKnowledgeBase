...
| [`docs/多端跨端-KMP方案.md`](多端跨端-KMP方案.md) | **共享层方案（推荐主线）**：KMP 逻辑共享 + 各端**原生 UI**；端口接口签名、性能/体积验收（含 JVM 实测 μs 级基准）、迁移 8 步、一致性机制 | 架构师 |
| [`docs/多端跨端-CMP方案评估.md`](多端跨端-CMP方案评估.md) | **CMP 评估**：正面回答"会不会影响原生体验"（16 项能力对照 + 12 条待实测门槛）、CMP 设计与端口接口、值得/不值得条件与回退；若选原生仍可复用的设计 | owner / 架构师 |
| [`docs/多端跨端-桌面端方案审计.md`](多端跨端-桌面端方案审计.md) | **桌面（Windows/Ubuntu）审计**：headless 边缘宿主设计（拓扑 A/B/C、鉴权默认、资源上限、服务化）、GUI 四选一、无目标 OS 时的验证手段、**发现 `DEVICE_ONLY` 契约漏洞 F-1** | owner / 架构师 |
| [`docs/多端跨端-工程议题（网络层·包体·热修复）.md`](多端跨端-工程议题（网络层·包体·热修复）.md) | **三条工程议题**：网络层是否统一到 libcurl（含各端 SDK 实测）、包体（实测 102 MB APK 构成）与插件化边界（iOS 只能下发资源）、热修复三层（策略/资源/代码）与四端可行性 | owner / 架构师 |
