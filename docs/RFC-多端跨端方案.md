...
|---|---|---|---|---|---|---|
| **Android** | Jetpack Compose（**已有，几乎不动**） | KMP `shared` | ONNX 嵌入（已有）+ 宿主 Ollama（已有，后续换 llama.cpp） | 已在（M0–M3 完成） | — | 模拟器自检 **26/26**、**186** 单测（功能 182 + 契约 4）、检索评测 ✅ |
...
| 5 | Kotlin 2.2.10 → **≥2.2.21**（**单独提交 + 回滚 tag**） | Android 全量复跑：186 测试 + 自检 26/26 + 检索评测 |
...
| 阶段 | 内容 | 工作量 | 验收（可证伪） |
|---|---|---|---|
| **M4** 共享层 | 契约夹具 ✅ + 去平台化 + `shared/` + iOS/Mac target + Kotlin 升级 | **8–12 天** | Android **186 测试 + 自检 26/26 + 检索数字 不回归**；`shared` 在 JVM 与 `iosSimulatorArm64` 均可编译并通过共享单测；**P1** 微基准 ≤100 µs（Kotlin/Native 版）、**P3** iOS 包体积增量 ≤5 MB |
...
| # | 风险 | 影响 | 概率 | 对策 / 回退 |
|---|---|---|---|---|
| R1 | 依赖可移植性（`org.json`/OkHttp/`java.*`/ONNX JVM API） | 共享层抽不出 | **高（确定发生）** | §5.2 步 2 先做，有 186 测试兜底 |
