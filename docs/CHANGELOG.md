...
- **职位分析默认关键词**：新增 `lastFetchKeyword`，删除/刷新职位时的缓存同步改用「最近一次实际采集」的关键词，修复输入框被编辑后污染缓存导致默认回填错误（Agent开发 vs Agent）。
- **批量分析默认展示**：保持「职位集合严格一致」才展示缓存报告的约束（职位列表须与批量分析严格一致，避免两份信息错位）。
