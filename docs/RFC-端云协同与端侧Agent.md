...
| D2 | 仓库形态 | ~~独立 repo `sekb-ondevice-agent`~~ → **修正为 monorepo**：端侧并入 SEKB 主仓库的 `apps/`（见《[实施记录](./RFC-端云协同-实施记录.md)》§17，2026-09-18）；SEKB 只加 §8 的 S1–S3 |
...
M0–M3 的实施记录（每轮做了什么、实测数字、踩的坑）已移到
**[RFC-端云协同-实施记录.md](./RFC-端云协同-实施记录.md)**。那份文件开头有一节
**「阶段总结与恢复指引」**（暂停点状态、待办、"怎么接着干"的命令、三条必须守住的不变量），
看当前进度直接跳那一节。

| 归档章节 | 内容 | 一句话结论 |
|---|---|---|
| §14 M0 | Mac 端侧跑通 + 单双档配置 | 端侧第一份**真实数字**（2B decode 86–116 tok/s）；配置用**生成**而非手抄，防漂移 |
| §15 M1 | 平面路由 + 6 类升级信号 + 交接摘要 + 版本戳 | 主链路**零调用点改动**即具备端云路由；关思考 25× |
| §16 M2 | Android 宿主（凭证/SSE/工具/权限/自检） | 模拟器 E2E 14/14；越权拦截率 50% |
| §17 仓库形态 | 端侧并入 `apps/`（D2 修正） | 一次 clone 拿全量；构建状态收进仓库，**不需要仓库外写权限** |
| §18 M3 | 端侧 RAG（嵌入/索引/工具/评测/量化） | Hit@1 87%、Hit@3 100%；int8 体积 1/4 且排序不变 |

---

## 相关文档

- [RFC-端云协同-实施记录.md](./RFC-端云协同-实施记录.md) —— M0–M3 实施记录 + **阶段总结与恢复指引**
- [ops/16-端云协同协议.md](./ops/16-端云协同协议.md) —— 端侧宿主 ↔ SEKB 接口契约
- [apps/README.md](../apps/README.md) —— 多端目录、按需编译、跨端逻辑分层
- [apps/android/docs/VERIFICATION.md](../apps/android/docs/VERIFICATION.md) —— 端侧验证记录（自检/评测/踩坑）
- [apps/android/docs/RETRIEVAL-EVAL.md](../apps/android/docs/RETRIEVAL-EVAL.md) —— 检索评测与阈值标定
- [RFC-自迭代闭环设计.md](./RFC-自迭代闭环设计.md) —— 发布与回滚策略
