...
## ⚠️ 猎聘：服务器 IP 已被风控封禁（2026-09-23 新发现）

**现状：猎聘源当前完全不可用**——搜索接口返回**裸拒绝** `{"flag":0}`（无 `code`/`msg`），
浏览器打开搜索页会 302 到验证页：

```
https://safe.liepin.com/page/liepin/captchaPage_ip_PC?euuid=ZmxhZzEwMDAxfGlwX3VhOjQ5LjIzMg...
页面文案：「猎聘安全中心发现您的帐号或 IP 可能存在异常…请进行安全验证」
```

`euuid` 解出来是 `flag10001|ip_ua:49.232...`——即**按 IP + User-Agent 标记**，不是账号问题。

**排除过的假设**（都不是原因）：

| 假设 | 实测 |
|---|---|
| 请求头不够像浏览器 | ❌ 补 `sec-ch-ua` / `Sec-Fetch-*` / `priority` 后仍 `{"flag":0}` |
| 短时高频限流 | ❌ 间隔 15s 重试仍 `{"flag":0}` |
| 接口契约变更 | ❌ 返回的是 `{"flag":0}` 裸拒绝，不是结构变化（`resp.json()` 正常解析） |
| Cookie/令牌缺失 | ❌ `acw_tc` / `__gc_id` / `XSRF-TOKEN` 三步流程与封禁前一致，仍被拒 |

**原因判断**：**今天为 FDE 全量扩搜连续跑了多轮 22 关键词扫描**，每轮对猎聘发 1 次首页 GET +
1 次（或 2 次含翻页）搜索 POST，累计请求量触发了阿里云 WAF 的 IP 级风控。**这是本次操作的副作用。**

**影响**：猎聘是唯一免登录可用的通用招聘平台（BOSS 需登录、智联虽可用但结果偏杂），
它一度为 FDE 集贡献了 49/101 条岗位（含滴滴、美团 CatPaw、锐捷、中石油等非大厂官方站覆盖的岗）。

**数据未丢失**：扩搜整合时用的是**并集**（`最终集 = 现有集 ∪ 新筛出集`），
所以即使猎聘这次返回 0，之前抓到的岗位仍保留在「第四层」搜索里。

**✅ 已修复（2026-09-23，见提交「采集源守卫」）**：新增
`backend/app/agents/job/source_guard.py`，提供节流 + 熔断 + 冷却后探测三件事。

复盘确认的**两个结构性原因**，逐一对应到修复：

| 原因 | 具体 | 修复 |
|---|---|---|
| **没有节流** | ①`fetch_all` 关键词之间**无延迟**；②更严重的是猎聘列表接口不带 JD，`_fetch_jds` 会为**每条职位**并发抓详情页（`_JD_CONCURRENCY=6`）→ 单关键词 `2 + 40` 次请求，一轮 22 关键词扫描近 **2000 次** | ①`_safe_fetch` 内按源档位节流（通用平台 4s / 官方站 0.8s，`_IntervalGate` 预约式、sync+async 共用状态）；②详情页并发 `6→2`、叠加 1s 间隔、**单次封顶 20 条** |
| **被拒后还在打** | 猎聘首次 `flag:0` 后只记 warning 返回空列表，**剩下 21 个关键词照打** → 把"临时标记"升级成"持续封禁" | `flag != 1` 改为抛 `SourceBlockedError`；`_safe_fetch` 捕获后计数，**达阈值即熔断**，冷却期内**一个请求都不发** |

**冷却后自动重试（半开探测）**：冷却期（默认 30 分钟）过后放**一次**探测请求——
成功则复位并记 info 日志，失败则重新打开。所以**风控自行解除后会自动恢复，不需要人工干预**；
状态持久化在 `data/source_guard.json`，**容器重启不会重置冷却**（否则重启即等于立刻再打一次）。

**运维旋钮**（环境变量，见模块 docstring）：`SEKB_JOB_BOARD_INTERVAL_S`(4.0) /
...
`SEKB_JOB_JD_CONCURRENCY`(2) / `SEKB_JOB_JD_MAX`(20)。

**仍未做（备选）**：
- 仿 BOSS 加猎聘**浏览器源**、人工过一次验证码后复用 Cookie（IP 标记仍在时浏览器同样被拦）；
- 若仍需更大扫描量，把 22 词一轮拆成多轮、每轮间隔几分钟。

**给后续操作的硬约束**：**不要再对猎聘做连续多关键词扫描**。扩搜时应优先用
「少量关键词 + 多轮间隔」，或先用智联/大厂官方站凑量，猎聘只做补充。
