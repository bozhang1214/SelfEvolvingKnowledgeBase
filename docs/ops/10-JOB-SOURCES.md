...
**可选解法（按推荐度）**：
1. **加节流**（推荐，治本）：给采集编排加「关键词 × 源」级的最小间隔 + 重试退避，
   避免单轮扫描把某个源打爆。当前 `JobCollector.fetch_all` 逐关键词串行，但同一关键词内
   各源是并发的，且不同关键词之间**没有延迟**。
2. **等风控自行解除**：阿里云 WAF 的 IP 标记通常有时效（数小时到数天），可观察后重试。
3. **走浏览器 + 人工过验证**：像 BOSS 那样加猎聘浏览器源，首次在浏览器里过掉验证码后
   复用 Cookie。成本较高，且 IP 标记仍在时浏览器同样会被拦。
4. **降关键词数**：把 22 词一轮拆成多轮、每轮间隔几分钟。
