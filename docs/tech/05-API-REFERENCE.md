...
> ⚠️ 现状：实际 `@router` 端点共 **84 个**<!-- fact:api_endpoints=84 -->（auth 8 / chat 3 / chat_share 3 / conversations 7 / **device 5** / **edge 3** / health 3 / job 20 / knowledge 6 / metrics 1 / monitoring 1 / news 6 / profile 2 / share 7 / upload 8；device 与 edge 是端云协同 S1/S2 新增的，2026-09-20 对齐实测）。T1 事实表已改为**生成物**（`scripts/gen_facts_routes.py`，`doc_guard` 第 6 项校验同步），与本文数字一致。
