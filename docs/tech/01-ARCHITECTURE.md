...
flowchart LR
    ...
    API -->|L2 记忆 + 用量统计| RD[(redis:7)]
    CLOUD[云端平台<br/>扣子/百炼/千帆/Dify] -->|HTTPS /jobcopilot| MCPH[jobcopilot-mcp 容器 :8765<br/>同镜像内核 HTTP/SSE 入口]
...
| 容器 | 镜像 | 端口/对外 | 职责 | 证据 |
|------|------|-----------|------|------|
| backend | self-evolving-kb-backend | 127.0.0.1:8000（仅内网） | FastAPI + LangGraph 全部逻辑；`--workers 1`；容器内 stdio 拉起 jobcopilot-mcp 子进程 | docker-compose.prod.yml:37-116；backend/Dockerfile |
| frontend | self-evolving-kb-frontend | 80/443 对外（nginx + HTTPS） | SPA 静态托管 + 反代 `/sekb/api` → backend；SSE 关缓冲 | docker-compose.prod.yml:119-161；deploy/nginx.conf |
| rsshub | diygod/rsshub | 1200（内网） | 资讯 RSS 源（含掘金鸿蒙标签等） | docker-compose.prod.yml:165-184 |
| browser | sekb-browser | 1300（内网） | BOSS 直聘采集（浏览器自动化） | docker-compose.prod.yml:189-214 |
| jobcopilot-mcp | self-evolving-kb-backend（复用镜像） | 127.0.0.1:8765 | 同一套内核的 **HTTP/SSE 入口**，供云端平台调用（BYOK，无内置 Key） | docker-compose.prod.yml:268-320 |
| postgres | postgres:16-alpine | 内网 | **预留未启用**（`profiles: ["with-db"]`，迁移到 PG 后端时才启动） | docker-compose.prod.yml:219-240 |
| redis | redis:7-alpine | 内网 | **已启用**：L2 中期记忆 + 对话 token/费用统计 | docker-compose.prod.yml:243-260；config.yaml:120-126, 205-208 |
| prometheus/grafana/loki/alertmanager | prom/grafana/loki 等 | 内网/监控 | 指标、看板、日志、告警 | docker-compose.monitoring.yml:26-214 |
...
flowchart TB
    ...
    subgraph 能力层[能力层]
        ...
        STO[storage: JSON/User/Share/ChatShare/Profile]
    ...
    subgraph 工具层[工具层]
        ...
        FP[tools/file_processor + image_processor]
    end
...
**依赖注入与装配**：`AppContext` dataclass 聚合全部运行时组件，由 `initialize_app("config.yaml")` 一次性装配（bootstrap.py:57-106, 133-356）；`get_app_context()` 提供全局访问（bootstrap.py:358-363），API 路由通过它取组件。

**装配顺序**（bootstrap.py:133-356）：配置 → JWT 强度校验（fail-fast）→ 日志 → tracing → LLM 工厂 → JSON 存储 → L1 记忆 → L2 Redis 中期记忆（条件）→ Prompt 注册表 → 对话用量统计（条件，Redis）→ 工具注册表（起 MCP 子进程）→ 反思策略 → L3 ChromaDB（条件）→ 知识入库引擎（条件）→ 用户/分享存储 → LangGraph 图 → news/job Agent（条件；job 启用时预热内核 MCP 连接）。

### 3.2 前端分层

```mermaid
flowchart LR
    P[pages: Login/Register/Home/Chat/Files/Knowledge/News/Job/Settings/Share*]
...
| LLM 模型 | deepseek-flash（全角色统一） | — | 按角色配温度/max_tokens；11 个角色（config.yaml:27-82） | 其它闭源/开源 | 原 chat/reasoner 分级已统一（见 ADR-05 现状） |
...
| 鉴权 | JWT（自研签发） | — | 轻量；7 天（168h）过期 + 前端滑动续租 | OAuth 托管 | 无 refresh 黑名单（见 BACKLOG） |
...
- **决策**：用 LangGraph `StateGraph`，节点=Supervisor/Planner/Executor/Critic/Scribe + chat_simple/clarify/rag_retrieval/rewrite，显式边与条件路由（backend/app/graph/builder.py:305-410）。
...
- **决策**：JSONStorage（`data/index.json` + `data/conversations/{id}.json`），`asyncio.Lock` + `tmp/os.replace` 原子写（json_storage.py:77, 122-141, 166-190）；compose 中 postgres:16 **预留未启用**（docker-compose.prod.yml:219-240）。
...
- **决策**：`CMD --workers 1`（backend/Dockerfile），端口绑 `127.0.0.1:8000:8000`（docker-compose.prod.yml:61-63）。
...
- **决策**：`DirectVectorStore` 直连 ChromaDB（tools/direct/vector_store.py:30-90）；MCP 只用于跨边界能力——`web_search`（registry.py:99-195）与 JobCopilot 内核分析（agents/job/mcp_client.py:40，stdio 长连接）。
- **理由**：本地调用 <10ms；工具层内外分治（本地直连高频、MCP 对外）。
- **后果**：知识库与 registry 两套抽象并存，需维护两处实现。
- **替代**：全量 MCP（统一但慢）、全量直连（外部工具 SSRF/凭据边界风险）。

### ADR-05：为什么按角色固定模型档（原 chat/reasoner 分级，现统一 flash）
- **背景**：不同任务思考深度不同；reasoner 贵且慢。
- **决策**：模型档按角色固定在 `config.yaml` 的 `llm.roles`（config.yaml:27-82）。**现状（7852aae 起）**：11 个角色**全部**为 `deepseek-flash`，原 planner/critic_complex 的慢思考位已统一；`reasoner_to_chat` 降级链配置仍在（config.yaml:84-86），但已无角色声明 reasoner，实际不会触发（llm_factory.py:513-527）。`cost_control` 仅保留定价表与 token/费用统计（预算占位已按 D3 删除，config.py:224-228）。
- **理由**：质量/成本/延迟三角；按角色固定档位便于统一调参与降级。
- **后果**：planner 段曾是首 token 延迟主因（15s+，reasoner 时期实测，见 02 TTFT 分析）。
- **替代**：全 chat（快但规划弱）/ 全 reasoner（贵且慢）。

### ADR-06：同步/异步边界划在哪
- **背景**：知识入库、偏好抽取等不应阻塞主回复。
- **决策**：主对话全异步（FastAPI + LangGraph 流式）；知识自迭代入库在回复路径内 `await`（chat.py:321-334），后台偏好抽取由 `asyncio.create_task` 调度（profile_service.py:153-166）、上传入库走 Starlette `BackgroundTasks`（upload.py:215 等；T7 详述）。
...
|------|-----------|---------|--------------|
| 性能（TTFT） | 复杂查询首字 27-35s；简单 5-8s | 流式生成；异步；`--workers 1` | planner 段主导延迟（现为 deepseek-flash，数字系 reasoner 时期实测，见 02/ADR-05）；RAG 21s 曾现（T7/02） |
| 可用性 | 重试 + 降级 | tenacity（非流式）；模型降级链（reasoner→chat，当前无角色命中）；异常脱敏返回 error_id | 流式无重试；静默降级点偏多（见 07） |
...
| 成本 | token 预算控制 | cost_control：定价表 + 对话 token/费用统计（Redis） | 预算占位已删（config.py:224-228，D3）；已接入 13/23 调用点，10 处绕过统计（T3） |
...
- postgres 容器仍为**预留未启用**（`profiles: ["with-db"]`，默认不启动）；redis 自 2026-09-10 起**已启用**（L2 中期记忆 + 对话用量统计），本文按「已启用」描述。
...
- [03-MODULES.md](./03-MODULES.md)（§11.5 SEKB↔JobCopilot 内核接入）
