...
| 层 | 数量 | 技术栈 | 说明 |
|----|------|--------|------|
| 后端单元 | 528 用例（29 文件） | pytest + pytest-asyncio + respx | 覆盖 config/graph/agent/storage/memory/news/job/state 等 |
| 后端集成 | 16 | pytest | storage+memory 集成、eval pipeline |
| 后端 API 路由级 | 7 | FastAPI TestClient | test_api_client.py（知识列表/搜索/401/403） |
| 前端单元 | 64 | vitest + @testing-library/react | api/auth/chat-store/file/logger/user/home 组件 |
...
| 前端 | api/chat-store/file/logger/user/home | store 会话隔离、SSE、上传、日志脱敏 |
...
| 指标 | 9 项量化：intent_confidence/plan_step_count/replan/tool_success/groundedness/coherence/relevance/e2e_latency/token |
| 判级 | eval/metrics.py 读 evaluation.metrics.*.{good,warn} |
| 运行 | `python -m app.cli.main eval --dataset …`；CI 中 eval job 仅 main+定时，**非阻断**（`|| echo`） |
...
**质量现状**：CI 阻断项覆盖 lint/类型/单测/集成/构建/生产依赖 audit；覆盖率门禁 40%（当前 43%+）。
...
| 短板 | 现状 | 建议 |
|------|------|------|
| 覆盖率 | 43%（门禁 40%） | 补 upload/job/news 路由测试；逐轮上调 |
