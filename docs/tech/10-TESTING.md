...
| 层 | 数量 | 技术栈 | 说明 |
|----|------|--------|------|
| 后端单元 | 978 用例（62 文件）<!-- fact:backend_unit_cases=978 --> | pytest + pytest-asyncio + respx | 覆盖 config/graph/agent/storage/memory/news/job/state/plane_router 等 |
...
**全量口径**：`pytest tests/` = **994**（978 单元 + 16 集成；API 路由级 7 条在单元目录内，
