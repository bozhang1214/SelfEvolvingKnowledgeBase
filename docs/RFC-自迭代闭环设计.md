...
related: [docs/tech/00-README, docs/ops/00-README, docs/ops/14-CHANGE-RELEASE-POLICY]
---

> **版本库归属（2026-09-16）**：本文原在本地讨论区 `docs/tmp/`（gitignore，不进版本库），
> 但 `docs/ops/12-GITEA.md` 与 `docs/ops/14-CHANGE-RELEASE-POLICY.md` 都**以它为决策依据**——
> 放在未版本化的目录里等于「引用了一份别人拿不到的文件」。现已移入版本库；
> 状态仍为 `draft`（讨论稿），改动请直接改本文件。
