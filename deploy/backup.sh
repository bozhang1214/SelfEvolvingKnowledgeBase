...
# ============================================================
# ⚠️ 已废弃（2026-09-14）：请使用 scripts/backup_kb.sh
#
# 保留本文件仅为记录历史，**deploy.sh 已不再调用它**。原因：
#   1) 写死 /backup 目录，部署用户 bo 对其无权限 → 调用必失败；
#   2) 会把 .env.prod 明文打进备份并 sync 到 S3（安全审查 SEC-09，待修）；
#   3) 不覆盖 sekb_gitea_data 数据卷（RFC D-02 起它是版本管理权威源）。
# scripts/backup_kb.sh 覆盖 sekb_data + sekb_gitea_data，且带 EXIT trap 兜底重启服务。
#
# ============================================================
