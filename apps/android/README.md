...
一句话：**191 个 JVM 单测**（功能 187 + 跨端契约夹具 4）+ 模拟器自检 **28 PASS / 0 FAIL / 2 SKIP**
（真实 Ollama 流式、越权拦截率 50%、云端 SSE 执行位置、路由事件落库、端侧 RAG 检索评测 87%/100%/0.928）
+ 工具调用合法率对照实验（40 次调用四种条件全 100%）。
契约夹具见 [`../contract/README.md`](../contract/README.md)——四端跑同一份 JSON，改夹具会立刻触发重跑。
