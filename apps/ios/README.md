...
> ⚠️ **2026-09-20 更新（UI 方案待拍板）**：下面 M4 计划里写的「SwiftUI 界面」是原方案；
> [`docs/RFC-多端跨端方案.md`](../../docs/RFC-多端跨端方案.md) §4 D1 建议改为
> **Compose Multiplatform 一套 UI 覆盖 Android/iOS/鸿蒙/桌面**（CMP 1.9.3 已支持 AGP 9，iOS 有原生 IME/帧率能力），
> 并把「SwiftUI」降级为**回退方案**（仅当 CMP 在 iOS 上出现输入法/长列表/流式渲染三类致命问题时，
> 只换 UI 层，KMP 逻辑层一行不动）。**待 owner 确认 Q1 后据此更新本节。**
